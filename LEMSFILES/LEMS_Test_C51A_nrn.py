...
Components:
    ...
    C51A (Type: cell)
    iclamp0 (Type: pulseGenerator:  delay=0.05 (SI time) duration=0.2 (SI time) amplitude=1.5E-10 (SI current))
    network_of_C51A (Type: networkWithTemperature:  temperature=310.15 (SI temperature))
    Test (Type: Simulation:  length=0.3 (SI time) step=2.5000000000000002E-8 (SI time))
...
class NeuronSimulation():
    ...
    def __init__(self, tstop, dt, seed=12345):
        ...
        h("objref Iclamp_0")
        h("a_population_of_C51A[0].Soma { Iclamp_0 = new iclamp0(0.12592101) } ")
        ...
        # ######################   File to save: Test.v.dat (voltage_traces)
        # Column: population_of_C51A/0/C51A/0/v
        h(' objectvar v_0_voltage_traces ')
        h(' { v_0_voltage_traces = new Vector() } ')
        h(' { v_0_voltage_traces.record(&a_population_of_C51A[0].Soma.v(0.12592101)) } ')
        h.v_0_voltage_traces.resize((h.tstop * h.steps_per_ms) + 1)
    ...
    def save_results(self):
        ...
        # ######################   File to save: Test.v.dat (voltage_traces)
        py_v_0_voltage_traces = [ float(x  / 1000.0) for x in h.v_0_voltage_traces.to_python() ]  # Convert to Python list for speed, variable has dim: voltage

        f_voltage_traces_f2 = open('Test.v.dat', 'w')
        num_points = len(py_v_time)  # Simulation may have been stopped before tstop...

        for i in range(num_points):
            f_voltage_traces_f2.write('%e\t%e\t\n' % (py_v_time[i], py_v_0_voltage_traces[i], ))
        f_voltage_traces_f2.close()
        print("Saved data to: Test.v.dat")
...
if __name__ == '__main__':
    ...
    ns = NeuronSimulation(tstop=300, dt=2.5e-05, seed=12345)
